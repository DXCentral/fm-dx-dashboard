...
if 'map_key' not in st.session_state: st.session_state.map_key = 2100
...
# 2. DATA LOADING (FIXED 403 PERMISSIONS)
@st.cache_data(ttl=2592000)
def load_data():
    try:
        creds_dict = dict(st.secrets["gcp_service_account"])
        creds_dict["private_key"] = creds_dict["private_key"].replace("\\n", "\n")
        # RESTORED DRIVE SCOPES TO PREVENT 403
...
# 6. ES-CLOUD TRACKER (RESTORED PLAYBACK ENGINE)
    ...
    if not map_df.empty:
        ...
        if view_mode == "Heatmap":
            layers.append(pdk.Layer('HeatmapLayer', data=r_df[['Mid_Lat', 'Mid_Lon']].dropna(), get_position='[Mid_Lon, Mid_Lat]', radius_pixels=65))
        else:
            layers.append(pdk.Layer('LineLayer', data=r_df[[dx_lat, dx_lon, st_lat, st_lon]].dropna(), get_source_position=f'[{dx_lon}, {dx_lat}]', get_target_position=f'[{st_lon}, {st_lat}]', get_width=1))
