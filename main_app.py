...
states = {
    ...
    'selected_year': None, 'sel_alm_d': None, 'sel_alm_y': None,
    'map_key': 500000, 'hour_map_key': 600000, 'year_map_key': 700000,
    'dist_map_key': 800000, 'alm_key': 900000
}
...
def load_data():
    try:
        ...
        return pd.DataFrame(), "Distance", "Distribution", None, None, None, None, "DXer", "Hour", "Year", "Day", "Month"
...
# 6. ES-CLOUD TRACKER
    ...
    vm = st.pills("MAP LAYER", ["Es Cloud Heatmap", "Path Analysis"], default="Es Cloud Heatmap")
    ...
    if not map_df.empty:
        ...
        layer = pdk.Layer(
            ...
            radius_pixels=60, intensity=1.5, threshold=0.05
        )
        st.pydeck_chart(pdk.Deck(map_style='mapbox://styles/mapbox/dark-v10', initial_view_state=pdk.ViewState(latitude=38, longitude=-95, zoom=3.5), layers=[layer], height=800))

# 7. GEOGRAPHIC ANALYSIS
elif selected_page == "GEOGRAPHIC ANALYSIS":
    st.header("Geographic Analysis")
    gv = st.pills("MODULE", ["US States", "Canada", "International", "Distance Hub"], default="US States")
    ...
    if gv == "Distance Hub":
        ...
        with col_m:
            ...
            fig = px.bar(d_counts, x='Logs', y=dd_col, orientation='h', color='Logs', color_continuous_scale='Reds', template="plotly_dark")
            ev = st.plotly_chart(fig, use_container_width=True, on_select="rerun", key=f"d_hub_{st.session_state.dist_map_key}")
            if ev and ev.get("selection") and ev["selection"].get("points"):
                st.session_state.selected_tier = ev["selection"]["points"][0]["y"]
                st.rerun()
        ...
        if st.session_state.selected_tier:
            with col_f:
                ...
                st.metric("Logs in Tier", f"{len(s_of):,}")
...
# 8. TEMPORAL TRENDS
    ...
    tv = st.pills("MODULE", ["Hourly", "Monthly Almanac", "Yearly"], default="Hourly")
    ...
    if tv == "Hourly":
        ...
        with col_m:
            ...
            ev = st.plotly_chart(fig, use_container_width=True, on_select="rerun", key=f"h_chart_{st.session_state.hour_map_key}")
            if ev and ev.get("selection") and ev["selection"].get("points"):
                st.session_state.selected_hour = int(ev["selection"]["points"][0]["x"])
                st.rerun()
        ...
        if not m_df.empty:
            ...
            for i, r_name in enumerate(full_matrix.index):
                for j, c_name in enumerate(full_matrix.columns):
                    ...
                    is_peak = (isinstance(r_name, int) and 1 <= r_name <= 31 and c_name in core_y and val/max_val > 0.8)
            ...
            if st.session_state.sel_alm_d:
                with c_intel:
                    ...
                    if not s_day.empty:
                        st.metric("Total Logs", len(s_day))
                        st.metric("MUF Reported", f"{s_day['Frequency'].max()} MHz")
        ...
        y_data = filt_df.groupby(y_col).size().reset_index(name='Logs')
        fig = px.bar(y_data, x=y_col, y='Logs', template="plotly_dark", color_discrete_sequence=['#D32F2F'])
