...
df, last_date_glob, d_col, dd_col, dx_lat, dx_lon, st_lat, st_lon, dx_loc_col, h_col, y_col, dom_col, m_name_col = load_data()
if df.empty: st.stop()
...
# 6. ES-CLOUD TRACKER
    ...
    with hc1:
        ...
        play_speed = st.selectbox("Playback Speed", options=list(speed_sets.keys()), index=2)
    ...
    if gv == "Distance Stats":
        ...
        if st.session_state.selected_tier:
            with col_f:
                ...
                st.markdown('<div class="stat-header">UNIQUE DXERS</div>', unsafe_allow_html=True); st.markdown(f'<div class="stat-val">{s_of["DXer"].nunique()}</div>', unsafe_allow_html=True)
                ...
                st.markdown('<div class="stat-header">ORIGIN HOTSPOTS</div>', unsafe_allow_html=True); st.dataframe(s_of.groupby(dx_st_col).size().reset_index(name='L').sort_values('L', ascending=False).head(5), hide_index=True)
                ...
                st.markdown('<div class="stat-header">FURTHEST</div>', unsafe_allow_html=True); f = s_of.sort_values(d_col, ascending=False).iloc[0]; st.markdown(f'<div class="stat-val">{f[d_col]:,.0f} MILES</div><div class="stat-label">{f["Station"]} caught by {f["DXer"]}</div>', unsafe_allow_html=True)
    else:
        ...
        if st.session_state.selected_state:
            with col_f:
                ...
                st.markdown('<div class="stat-header">FURTHEST RECEPTION</div>', unsafe_allow_html=True); f = s_of.sort_values(d_col, ascending=False).iloc[0]; st.markdown(f'<div class="stat-val">{f[d_col]:,.0f} MILES</div><div class="stat-label">{f["Station"]} caught by {f["DXer"]} at {f["Local_Time"]}</div>', unsafe_allow_html=True)
        ...
        if st.session_state.selected_hour is not None:
            with col_f:
                ...
                if not s_h.empty:
                    ...
                    st.markdown('<div class="stat-header">MUF REPORTED</div>', unsafe_allow_html=True); st.markdown(f'<div class="stat-val">{s_h["Frequency"].max()} MHz</div>', unsafe_allow_html=True)
