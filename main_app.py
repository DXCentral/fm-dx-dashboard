...
st.markdown("""
    ...
    [data-testid="stSidebar"] { background-color: #0A0A0A; border-right: 1px solid #1A1A1A; min-width: 300px !important; }
...
def load_data():
    try:
        ...
        # Standardize Names & Clean Coordinates
        ...
        # Force numeric and drop actual NaNs before calculating midpoint
...
if selected_page == "DASHBOARD OVERVIEW":
    ...
    if not day_df.empty:
        ...
        if view_mode == "Midpoint Heatmap (Es-Cloud)":
            # DROP NA FOR HEATMAP
            map_data_heat = map_data.dropna(subset=['Mid_Lat', 'Mid_Lon'])
            layers.append(pdk.Layer(
                'HeatmapLayer', data=map_data_heat, get_position='[Mid_Lon, Mid_Lat]',
            ...
            # DROP NA FOR ARCS
            map_data_arc = map_data.dropna(subset=['DX_Lat', 'DX_Lon', 'ST_Lat', 'ST_Lon'])
            path_counts = map_data_arc.groupby(['DX_Lat', 'DX_Lon', 'ST_Lat', 'ST_Lon']).size().reset_index(name='density')
            layers.append(pdk.Layer(
