...
if 'map_key' not in st.session_state: st.session_state.map_key = 30000
...
# 6. MODULE 2: ES-CLOUD TRACKER (RESTORED PLAYBACK ENGINE)
...
# 7. MODULE 3: GEOGRAPHIC ANALYSIS (LOCKDOWN PERFORMANCE BUILD)
    ...
    if target:
        ...
        with col_m:
            ...
            if ev and ev.get("selection") and ev["selection"].get("points"):
                ...
                if target == 'Canada': 
                    inv = {v: k for k, v in ca_name_map.items()}
