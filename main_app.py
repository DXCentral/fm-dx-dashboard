...
if 'map_key' not in st.session_state: st.session_state.map_key = 400000
...
def load_data():
    try:
        ...
        # 🛡️ THE CASE-INSENSITIVE SHIELD (Fixes 2025 International Logs)
        ...
        # 🌍 MIDPOINT ENGINE (Calculates if missing, otherwise preserves)
    ...
    if gv == "Distance Stats":
        ...
        with col_m:
            if f_dx_loc != "All Locations":
                ...
                for t, colors in tier_colors.items():
                    ...
                    if not t_data.empty:
                        r_layers.append(pdk.Layer('HeatmapLayer', data=t_data, get_position=f'[{st_lon}, {st_lat}]', radius_pixels=60, intensity=1, color_range=[c + [40] for c in colors]))
        ...
        if st.session_state.selected_tier or f_dx_loc != "All Locations":
            with col_f:
                ...
                st.dataframe(s_of.groupby(['Frequency', 'Station']).size().reset_index(name='L').sort_values('L', ascending=False).head(5), hide_index=True)
