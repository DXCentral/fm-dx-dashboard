...
if 'map_key' not in st.session_state: st.session_state.map_key = 800
...
with st.sidebar:
    ...
    selected_page = option_menu(menu_title="DATA MODULES", options=["DASHBOARD OVERVIEW", "ES-CLOUD TRACKER", "GEOGRAPHIC ANALYSIS", "TEMPORAL TRENDS", "FREQUENCY & MUF", "STATION & RDS IQ", "RECEPTION DYNAMICS"], icons=["house-fill", "cloud-haze2", "geo-alt", "clock-history", "graph-up-arrow", "broadcast-pin", "diagram-3"], default_index=1)
...
# 5. MODULE 2: ES-CLOUD TRACKER (SAFE SUBSETTING FIX)
if selected_page == "ES-CLOUD TRACKER":
    ...
    if not map_df.empty:
        ...
        # V2.1 FIXED LOGIC: Pass ONLY coordinates to Pydeck to prevent JSON crashes
...
# 6. MODULE 3: GEOGRAPHIC ANALYSIS (RE-ORDERED INTEL)
    ...
    if geo_view == "US States":
        # Detective Work: BigQuery Column Names
