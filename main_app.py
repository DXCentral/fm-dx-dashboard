...
# --- 2. DATA LOADING (FORBIDDEN FIX + COORDINATE SANITIZER) ---
...
def load_data():
    try:
        ...
        # Lat/Lon Detection & Sanitization (TYPE ERROR FIX)
        ...
        df = df.dropna(subset=[st_lat, st_lon]) # Drop NaNs so Pydeck doesn't crash
...
if not st.session_state.full_screen:
    ...
    with st.expander(label="GLOBAL FILTERS", expanded=True):
        ...
        f_state = r1[3].selectbox("State", ["All"] + sorted(df['State'].dropna().unique().astype(str).tolist()), key=f"te_{rk}")
        f_country = r1[4].selectbox("Country", ["All"] + sorted(df['Country'].dropna().unique().astype(str).tolist()), key=f"co_{rk}")
        ...
        f_month = r2[2].selectbox("Month", ["All"] + sorted(df['Local_Month'].dropna().unique().astype(str).tolist()), key=f"mo_{rk}")
        f_year = r2[3].selectbox("Year", ["All"] + sorted(df['Local_Year'].dropna().unique().astype(str).tolist()), key=f"yr_{rk}")
        ...
        f_reg = r3[1].selectbox("DXer Region", ["All"] + sorted(df['DXer_Region'].dropna().unique().astype(str).tolist()), key=f"rg_{rk}")
...
# --- 6. ES-CLOUD TRACKER (RESTORED PLAYBACK ENGINE) ---
    ...
    if not map_df.empty:
        ...
        if view_mode == "Es Cloud Location Heatmap":
            layers.append(pdk.Layer('HeatmapLayer', data=render_df[['Mid_Lat', 'Mid_Lon']], get_position='[Mid_Lon, Mid_Lat]', radius_pixels=65, intensity=2.0, color_range=[[183, 28, 28, 60], [211, 47, 47, 150], [244, 67, 54, 200], [255, 235, 238, 230], [255, 255, 255, 255]]))
        else:
...
# --- 7. GEOGRAPHIC ANALYSIS (REBUILT FROM BLUEPRINT) ---
    ...
    with tab1: # Country Stats - FILTERED TO EXCLUDE USA
        intl_df = filt_df[filt_df['Country'] != 'USA']
        st.subheader("International Log Distribution")
        ...
        # Stacked Bar: Intl Country by Month
        intl_month = intl_df.groupby(['Country', 'Local_Month_Name']).size().reset_index(name='Logs')
        fig = px.bar(intl_month, x="Local_Month_Name", y="Logs", color="Country", barmode="stack", color_discrete_sequence=px.colors.sequential.Reds_r)
    ...
    with tab2: # Canadian Stats
        ...
        col_can1.dataframe(prov_logs, column_config={"Logs": st.column_config.ProgressColumn("Volume", min_value=0, max_value=int(prov_logs['Logs'].max() if not prov_logs.empty else 1))}, hide_index=True)
    ...
    with tab3: # US Stats
        ...
        st.plotly_chart(px.bar(us_state_logs, x='State', y='Logs', color_discrete_sequence=['#D32F2F']).update_layout(bargap=0.4, paper_bgcolor='black', plot_bgcolor='black', font_color='white'), use_container_width=True)
