...
# Ensure Plotly charts look sleek
st.markdown("""
    <style>
...
# 2. DATA LOADING (FIXED Drive Scope for Forbidden Error)
...
def load_data():
    try:
        ...
        # KEY FIX: Added Drive Readonly scope to allow BigQuery to read Google Sheets
...
if selected_page == "DASHBOARD OVERVIEW":
    ...
    st.header("Geographic Footprint Analysis")
    
    with st.popover("📊 VIEW ANALYSIS"):
        st.markdown(f"Analysis for **{len(filt_df):,}** logs. Average skip distance: **{filt_df[d_col].mean():.0f} miles**.")

    # Floating Overlay Counters
    st.markdown(f"""
        <div style="position: relative; background-color: rgba(0,0,0,0.85); border: 1px solid #D32F2F; 
                    padding: 15px; border-radius: 5px; margin-bottom: -100px; z-index: 1000; width: fit-content; margin-left: 20px;">
            <span style="color: #D32F2F; font-size: 28px; font-weight: bold;">{filt_df['State'].nunique()}</span>
            <span style="color: white; font-size: 14px; margin-right: 20px;"> STATES/PROV</span>
            <span style="color: #D32F2F; font-size: 28px; font-weight: bold;">{filt_df[d_col].mean():.0f}</span>
            <span style="color: white; font-size: 14px;"> AVG MILES</span>
        </div>
    """, unsafe_allow_html=True)

    # Radius Map (Station Centric)
    st.pydeck_chart(pdk.Deck(
        map_style='mapbox://styles/mapbox/dark-v11',
        initial_view_state=pdk.ViewState(latitude=38, longitude=-95, zoom=3.5),
        layers=[pdk.Layer('ScatterplotLayer', filt_df, get_position=f'[{st_lon}, {st_lat}]', get_color='[211, 47, 47, 160]', get_radius=20000, pickable=True)]
    ))
    ...
    with c1:
        ...
        lead_df = filt_df.groupby('DXer').agg(Logs=('DXer', 'count'), Max_Dist=(d_col, 'max')).sort_values('Logs', ascending=False).reset_index()
        st.dataframe(lead_df, column_config={
            "DXer": "DXer",
            "Logs": st.column_config.ProgressColumn("Total Logs", format="%d", min_value=0, max_value=int(lead_df['Logs'].max() if not lead_df.empty else 1)),
            "Max_Dist": st.column_config.NumberColumn("Furthest", format="%d mi")
        }, hide_index=True, use_container_width=True)

    with c2:
        st.markdown("### DISTANCE DISTRIBUTION")
        fig = px.histogram(filt_df, x=d_col, nbins=30, color_discrete_sequence=['#D32F2F'])
        fig.update_layout(bargap=0.3, paper_bgcolor='black', plot_bgcolor='black', font_color='white', font_family='Oswald', xaxis_title="Miles", yaxis_title="Logs")
